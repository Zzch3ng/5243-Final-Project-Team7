...
# Find the project root automatically, even when this app lives in a subfolder.
PROJECT_ROOT = next(
    path for path in [Path(__file__).resolve().parent, *Path(__file__).resolve().parents]
    if (path / "models").exists() and (path / "data" / "processed").exists()
)
